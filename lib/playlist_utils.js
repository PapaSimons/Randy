...
module.exports = {
    ...
    initPlaylist: async function (num, emitplaylist, emitsticky){
...
function isAudioStream(link){
    return new Promise(function(resolve, reject) {
        ...
        if (!link){ return false };
        var need = needle.get(link, {open_timeout: 3000, follow_max: 5});
        need.on('header', function(status, headers) {
            console.log(headers);
            console.log("test substring - " + filetypesarr.some(v => headers['content-type'].includes(v.substring(1))));
            //return
            if (!headers.hasOwnProperty('content-type')) {
                resolve(false);
            } else {
                resolve(headers['content-type'].startsWith('audio') || filetypesarr.some(v => headers['content-type'].includes(v.substring(1))));
            }
        });
