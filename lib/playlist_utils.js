...
module.exports = {
    ...
    setMusicFolder: async function(mf) {
        if (!fs.existsSync(mf)){
            ...
            sdb.delete('status');
            pdb.delete('playlist');
            settings.musicfolder = mf;
            odb.set('settings',settings);
            await this.getAllSongs();
            cacheAllSongs();
            return await this.initPlaylist(3); 
        }
    },
    ...
    getAllSongs: function(){
        ...
        return new Promise(function(resolve, reject) {
            ...
            if (adb.has('allsongs')){
                ...
                var musicfolder = settings.musicfolder;
                filehound
                  .path(musicfolder)
                  .ext(filetypesarr)
    ...
    searchSongs: function(keyword){
        ...
        console.log("search with keyword " + keyword + " - found files: " + rtn.files.length + " , albums: " + rtn.albums.length);
        return rtn;
    },
    ...
    initPlaylist: async function (num){
        ...
        return setplaying(status.playing);
    },
    ...
    addlast: async function (link){
        if (isStream(link)){
            randylist.push(await getMeta(link));
        } else if (fs.statSync(link).isDirectory()){
            var alb = this.getAlbum(link).files;
            for (var i = 0; i < alb.length; i++){
                randylist.push(await getMeta(alb[i].path));
    ...
    addtosticky: async function (link){
        var obj = null;
        if (isStream(link)){
            obj = await getMeta(link);
        } else {
            var pp = path.parse(link);
            obj = {name:pp.name, path:link};
        }
        //check dup
        for (var i = 0; i < stickylist.length; i++){
            if (stickylist[i].path == obj.path){
                stickylist.splice(i, 1);
            }
        }
        stickylist.unshift(obj);
    }
};
            ...
            mm.parseFile(String(rs),{skipCovers:true})
              .then(function (metadata) {
                ...
                if (cov !== null){ //check folder album art first
                    var mobj = {songfile:rs,type:"file",tags:metadata,albumart:cov};
                    cdb.set(rs,mobj);
                    resolve(mobj);
                } else { //check online version
                    albumArt(metadata.common.artist, {album:metadata.common.album, size:"mega"} , ( error, response ) => {
                        var mobj = {songfile:rs,type:"file",tags:metadata,albumart:response};
                        cdb.set(rs,mobj);
                        resolve(mobj);
                    });
                }
...
function checkFolderAlbumArt(rs){
    ...
    var covpath = path.join("/mnt",cov.replace(settings.musicfolder,""));
