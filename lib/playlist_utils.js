...
var radioURL = require('./radio-url-finder.js');
var maxRadioAttemps = 5;
var maxRadioAttempsCounter = 5;
...
module.exports = {
    ...
    getSettings: function(){
        if (odb.has('settings')){
            ...
            //fill up missing settings for upgraded params
            for (i in tempsettings){
    ...
    getAllSongs: async function(){
            ...
            if (adb.has('allsongs')){
                ...
                if (alls.hasOwnProperty('songs')){
                    ...
                    return 'Problem loading all files';
                }
            } else {
                const musicfolders = settings.musicfolders;
                // 1. Convert your extensions into a glob pattern.
                // Example output: "**/*.{wav,mp3,flac,m4a}"
                // This allows fast-glob to search for all types in a single pass.
                const cleanExtensions = filetypesarr.map(ext => ext.replace('.', ''));
                const extensionPattern = `**/*.{${cleanExtensions.join(',')}}`;

                // 2. Map music folders to use the glob pattern
                // If musicfolders is an array like ['/Volumes/USB/Music'], 
                // this turns it into ['/Volumes/USB/Music/**/*.{wav,mp3,...}']
                const scanPatterns = musicfolders.map(folder => `${folder}/${extensionPattern}`);

                try {
                    // 3. Perform the blazing-fast async scan
                    const allsongs = await fg(scanPatterns, {
                        dot: false,         // Automatically ignore hidden files and directories
                        onlyFiles: true,    // We only want file paths, not folders
                        absolute: true,     // Returns full absolute paths like Filehound did
                        concurrency: 4,      // Limits hardware strain, adjust based on system
                        suppressErrors: true   // <-- CRITICAL: Prevents EACCES crashes on protected folders!
                    });
                    // 4. Save your results
                    adb.set('allsongs', { folder: musicfolders, songs: allsongs });
                    return allsongs;
                } catch (err) {
                    console.error("fast-glob error: " + err);
                    throw err;
                }
                /*
    ...
    setVolume: function(newvol){
        plobj.status.volume = newvol;
        sdb.set('status', plobj.status);
    },

    getVolume: function(){
        if (plobj.status.hasOwnProperty('volume')){
            return plobj.status.volume;
        } else {
            this.setVolume(100);
            return 100;
        }
...
function getRadios(keyword){
    return new Promise(async function(resolve, reject) {
        var rurl = await radioURL.getAPIURL();
        console.log("radio link " + rurl + '/json/stations/search/');
        fetch(rurl + "/json/stations/search", {
            method: "POST",
...
function getRadio(id){
    return new Promise(async function(resolve, reject) {
        var rurl = await radioURL.getAPIURL();
        console.log("radio link " + rurl + '/json/stations/byuuid/'+id);
        fetch(rurl + '/json/stations/byuuid/'+id, {
            method: "POST",
            ...
            console.error("getRadio error: " + error);
            resolve([]);
        });
    });
}
...
function isAudioStream(link){
    return new Promise(function(resolve, reject) {
        ...
        fetch(link, {method: 'GET', redirect: "follow", follow: 4}).then((response) => {
            console.log(response.headers.get('content-type'));
            var head = response.headers.get('content-type');
            resolve((head.startsWith('audio') || head == 'application/ogg'));
        });
    });
}
