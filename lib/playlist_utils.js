...
module.exports = {
    ...
    initPlaylist: async function (num){
        ...
        observe(plobj, 'status', function(newValue, oldValue){
            sdb.set('status', newValue);
...
function getRadios(keyword){
    return new Promise(function(resolve, reject) {
        needle.post('http://www.radio-browser.info/webservice/json/stations/search',
            {
                name: keyword,
                limit: 3
            }, 
            { json: true }, (err, res) => {
                ...
                for (var i=0; i<res.body.length; i++){
                    ...
                    arr.push({ name:radio.name, 
                                ...
                                path:{path:radio.url, site:radio.homepage}});
                }
...
function getMeta(rs){
    return new Promise(function(resolve, reject) {
        if (cdb.has(rs)){
            ...
            metaget.fetch(rs.site, function (err, meta_response) {
                if(err){
