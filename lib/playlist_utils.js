...
module.exports = {
    ...
    getAllSongs: async function() {
        ...
        if (adb.has('allsongs')) {
            const alls = adb.get('allsongs');
            if (alls && alls.hasOwnProperty('songs')) {
                console.log('found ' + alls.songs.length + ' songs');
                return alls.songs;
            } else {
                throw new Error('Problem loading all files');
            }
        }

        // 2. If not saved, prepare the glob patterns
        ...
        try {
            ...
            allsongs = await fg(scanPatterns, {
                dot: false,           // Automatically ignore hidden files and directories
                onlyFiles: true,      // We only want file paths, not folders
                absolute: true,       // Returns full absolute paths
                concurrency: 4,       // Limits hardware strain
                suppressErrors: true  // Prevents EACCES crashes on protected folders
            });
...
async function initRlist(){
    ...
    if (allsongs.length > Numofnextsongs){
        ...
        for (i in allsongs){
            rlist.push(await getMeta(allsongs[i]));
        }
    }
    return rlist;
}
