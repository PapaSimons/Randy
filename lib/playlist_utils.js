...
function getRadios(keyword){
    return new Promise(async function(resolve, reject) {
        ...
        //https://de1.api.radio-browser.info/json/stations/search - old api
        console.log("radio link " + rurl + '/json/stations/search/');
        needle.post(rurl + "/json/stations/search",
            {
                name: keyword,
                limit: 8
            }, 
            { json: true }, (err, res) => {
                if (err || res.statusCode != 200) {
                    ...
                    if (maxRadioAttempsCounter > 0){
                        maxRadioAttempsCounter--;
                        resolve(getRadios(keyword));
                    } else {
                        maxRadioAttempsCounter = maxRadioAttemps;
                        resolve([]);
                    }
                } else {
...
function getRadio(id){
    return new Promise(async function(resolve, reject) {
        ...
        needle.post(rurl + '/json/stations/byuuid/'+id,
            {},{ json: true }, (err, res) => {
                if (err || res.statusCode != 200) {
                    console.error("getRadio error: " + err);
                    if (maxRadioAttempsCounter > 0){
